...
## Perform DAPC (discriminant analysis of principal components)
    ...
    grp <- find.clusters(gen, max.n.clust=30)   # This will prompt two questions to determine the number of PCs to retain and the number of clusters to keep based on BIC scores
    dapc1 <- dapc(gen, grp$grp, n.pca=150, n.da=5)

    dapc.plot.df <- as.data.frame(dapc1$ind.coord)

    ggplot(data=dapc.plot.df, aes(x=LD, y=LD2, color=grp$grp))+
    geom_point(cex=3)+
    theme_classic()+
